...
# --- 1. إعدادات التصميم الاحترافي والمتجاوب للجوال ---
...
# دالة لتنظيف الأرقام (حذف البوينت إذا كان الرقم صحيحاً)
def fmt(num):
    if num == 0: return "0"
...
if st.button("🚀 بدأ التحليل الحسابي الكامل", use_container_width=True):
    ...
    # معالجة تحويل Min إلى Max
    is_min = (opt_type == "Minimize")
    cj_working = np.array(obj_coeffs + [0.0]*n_const)
    if is_min: cj_working *= -1

    # المرحلة التمهيدية: عرض الصيغة القياسية
    st.markdown("### 1️⃣ المرحلة التمهيدية: الصيغة القياسية")
    standard_html = "<div class='calc-container'>"
    
    if is_min:
        standard_html += f"<div style='margin-bottom: 10px;'><span style='font-size: 1.2rem;'>💡</span> <span style='color: #f1c40f; font-weight: bold;'>بضرب المعاملات في (-1) تم تحويل Minimize إلى Maximize.</span></div>"
    ...
    for i in range(n_const):
        ...
        # معالجة الناتج السالب (RHS)
        if curr_rhs < 0:
            standard_html += f"<div style='margin-bottom: 10px;'><span style='font-size: 1.2rem;'>⚠️</span> <span style='color: #f85149; font-weight: bold;'>القيد {i+1} ناتجه سالب ({fmt(curr_rhs)})، تم ضربه في (-1) لضبط الحل.</span></div>"
            curr_row *= -1
            curr_rhs *= -1
        ...
        standard_html += f"<p class='math-row' style='color: #ffcc00; font-weight: bold; margin-left: 10px;'>C{i+1}: &nbsp; {eq_text}</p>"
    
    standard_html += "</div>"
    st.markdown(standard_html, unsafe_allow_html=True)

    # تهيئة المصفوفة الأساسية
    ...
    for it in range(1, 11):
        ...
        # فحص الحل الأمثل (دائماً Max للعمليات العاملة)
        ...
        # --- العمليات الحسابية المفصلة (Zj والدلتا) ---
        ...
        calc_html += "<br><b>• حساب الدلتا Δj (صافي التقييم):</b><br>"
        for j in range(len(col_names)):
        ...
        if is_optimal:
            # إعادة تحويل Z إذا كانت المسألة Minimize
            final_res = -current_z_prime if is_min else current_z_prime
        ...
        st.markdown(f"""
            <div class='pivot-bar'>
                <span>📥 المتغير الداخل: <span class='tag'>{col_names[p_col_idx]}</span></span>
                <span>🎯 عنصر الارتكاز: <span class='pivot-val'>[{fmt(matrix[p_row_idx, p_col_idx])}]</span></span>
                <span>📤 المتغير الخارج: <span class='tag'>{basis[p_row_idx]}</span></span>
            </div>
        """, unsafe_allow_html=True)

        # تحديث المصفوفة (عمليات الصف البسيطة)
