...
if st.button("🚀 بدأ التحليل الرياضي الشامل", use_container_width=True):
    ...
    for it in range(1, 8):
        ...
        for j in range(len(col_names)):
            ...
            calc_html += f"<p dir='ltr' class='math-row'>• {col_names[j]}: {steps} = <span class='math-res'>{zj[j]:.2f}</span></p>"
        
        calc_html += f"<br><span class='math-title'>📊 حساب صافي التقييم (Zj - Cj):</span>"
        for j in range(len(col_names)):
            calc_html += f"<p dir='ltr' class='math-row'>• {col_names[j]}: {zj[j]:.2f} - {cj_full[j]} = <span class='math-res'>{deltas[j]:.2f}</span></p>"
        calc_html += "</div>"
